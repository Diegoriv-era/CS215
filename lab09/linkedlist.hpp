...
class LinkedList {
    ...
    public:
        ...
        void PrintBackwards(ostream& out); //prints from last node to the first
        void apply(void function(int& param)); //
        LinkedList operator+(const LinkedList& other) const; //combines both lists together and returns that new list.
        void clear(); //deletes everything in the linked list.
        bool exists(int v); // checks if it exists within the list. if it does it returns true.
